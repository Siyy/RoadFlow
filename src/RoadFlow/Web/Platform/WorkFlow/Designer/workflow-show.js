...
var wf_r=null;var wf_steps=[];var wf_texts=[];var wf_conns=[];var wf_option="";var wf_focusObj=null;var wf_width=108;var wf_height=50;var wf_rect=2;var wf_designer=false;var wf_connColor="#7e7e7f";var wf_connColor1="green";var wf_nodeBorderColor="#7e7e7f";var wf_noteColor="#efeff0";var wf_nodeBorderColor1="green";var wf_noteColor1="#e7f0c7";var wf_stepDefaultName="";var tempArrPath=[];var mouseX=0;var mouseY=0;var wf_json={};var wf_id="";var links_tables_fields=[];$(function()
{wf_r=Raphael("flowdiv",$(window).width()-10,$(window).height()-55);wf_r.customAttributes.type1=function(){};wf_r.customAttributes.fromid=function(){};wf_r.customAttributes.toid=function(){};});function addStep(x,y,text,id,isIn)
{var guid=getGuid();var xy=getNewXY();x=x||xy.x;y=y||xy.y;text=text||wf_stepDefaultName;id=id||guid;var rect=wf_r.rect(x,y,wf_width,wf_height,wf_rect);var fillcolor=wf_noteColor;var strokecolor=wf_nodeBorderColor;if(isIn)
{fillcolor=wf_noteColor1;strokecolor=wf_nodeBorderColor1;}
rect.attr({"fill":fillcolor,"stroke":strokecolor,"fill-opacity":1,"stroke-width":1,"cursor":"default"});rect.id=id;rect.type1="step";wf_steps.push(rect);var text2=text.length>8?text.substr(0,7)+"...":text;var text1=wf_r.text(x+52,y+25,text2);text1.attr({"font-size":"12px"});if(text.length>8)text1.attr({"title":text});text1.id="text_"+id;text1.type1="text";wf_texts.push(text1);}
...
function dragger()
{this.ox=this.attr("x");this.oy=this.attr("y");changeStyle(this);};Raphael.fn.drawArr=function(obj,isIn)
{if(!obj||!obj.obj1)
{return;}
if(!obj.obj2)
...
tempArrPath=[];obj.arrPath=this.path(path2);obj.arrPath.attr({"stroke-width":2,"stroke":isIn?wf_connColor1:wf_connColor});tempArrPath.push(obj);return;}
var point=getStartEnd(obj.obj1,obj.obj2);var path1=getArr(point.start.x,point.start.y,point.end.x,point.end.y,10);try
{if(obj.arrPath)
{obj.arrPath.attr({path:path1});}
else
{obj.arrPath=this.path(path1);obj.arrPath.attr({"stroke-width":2,"stroke":isIn?wf_connColor1:wf_connColor});if(wf_designer)
...
{var isIn=false;for(var j=0;j<taskJSON.length;j++)
...
{var id=taskJSON[x].stepid;var step=wf_r.getById(id);if(step)
